...
<table>
    ...
    @foreach(/** @var \App\Eloquent\Answer $answer */$report->getAttribute('answer') as $k => $answer)
        ...
        <tr>
            <td>{{$k + 1}}</td>
            <td>{{$answer->getAttribute('started_at')->formatLocalized('%d %B %Y %H:%M')}}</td>
            <td>{{$isInProcess ? 'Tahap Pengerjaan' : $answer->getAttribute('finished_at')->formatLocalized('%d %B %Y %H:%M')}}</td>
            @foreach (/** @var \App\Eloquent\QuestionCategory $category */$categories as $category)
                <?php
